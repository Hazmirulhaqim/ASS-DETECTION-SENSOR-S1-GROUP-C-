...
uint8_t buf[32];
float accbuff[BUFFERSIZE * NB_AXES] = {0};
float class_buff_output[CLASS_NUMBER]; // Buffer of class probabilities
	  ...
	  for (uint16_t i = 0; i < BUFFERSIZE; i++) {
		 ...
		 if (ret != HAL_OK) {
			 ...
			 if (ret != HAL_OK) {
				 ...
				 accbuff[NB_AXES * i] = ((buf[1] << 8) | buf[0]);
				 accbuff[(NB_AXES * i) + 1] = ((buf[3] << 8) | buf[2]);
				 accbuff[(NB_AXES * i) + 2] = ((buf[5] << 8) | buf[4]);
			 }
		 }
	 }

	 neai_classification(accbuff, class_buff_output, &newClass);
